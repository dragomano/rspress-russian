...
- **Тип**: `string`
- **По умолчанию**: [`siteOrigin`](/api/config/config-basic#siteorigin) + [`base`](/api/config/config-basic#base), либо `base`, если `siteOrigin` не задан
...
export default defineConfig({
  ...
  base: '/base/',
  plugins: [
    // По умолчанию `siteUrl` имеет значение `https://example.com/base/`
    pluginRss(),
  ],
});
```
