...
Обеспечивает интерактивную редактируемую площадку для предпросмотра компонентов в блоках MDX-кода.

:::tip
Используйте этот плагин совместно с [@rspress/plugin-preview](./preview). В отличие от `plugin-preview`, `plugin-playground` компилирует код в браузере, поэтому имеет больше ограничений. Например, он не может импортировать модули из локальных файлов. Используйте `plugin-playground` как дополнение к `plugin-preview`, когда требуется редактирование кода в реальном времени.
:::
...
1. Сейчас это работает только в файлах `.mdx`.
2. Экспортируйте компонент как `default`, Rspress отрендерит его автоматически.
3. Проверка типов для TSX сейчас не выполняется.
...
- **Тип:** [IStandaloneEditorConstructionOptions](https://microsoft.github.io/monaco-editor/typedoc/interfaces/editor_editor_api.editor.IStandaloneEditorConstructionOptions.html)
...
Для настройки вы можете обратиться к встроенному компоненту [Playground.tsx](https://github.com/web-infra-dev/rspress/blob/main/packages/plugin-playground/static/global-components/Playground.tsx).
