...
- **Обработка темного режима**: Внедрите `MutationObserver` для отслеживания изменений класса `html.dark`, затем синхронизируйте с `body.dark` или выполните другую обработку тёмного режима.
