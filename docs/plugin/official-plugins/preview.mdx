...
Предпросмотр компонентов из блоков кода в файлах MDX. Это полезно для документации библиотек компонентов.
...
2. Экспортируйте компонент как `default` (экспорт по умолчанию), и Rspress отрендерит его автоматически.

:::

### 3. Вынос кода компонента в отдельные файлы (не обязательно)

Вместо того чтобы писать код компонента прямо в блоке кода MDX, можно использовать его вместе с [блоком кода из файла](../../guide/use-mdx/code-blocks.mdx#file-code-block) и хранить примеры кода в отдельных файлах.
...
Режим предпросмотра iframe имеет отдельные среды компиляции и выполнения.

1. Отдельное окружение сборки: пример кода в блоке кода компилируется как точка входа отдельным экземпляром Rsbuild, что позволяет внедрять переменные Sass/Less и другую конфигурацию.

2. Отдельное окружение выполнения: исключаются конфликты стилей с сайтом документации, а библиотека компонентов может загружать собственный `base.css`.
...
- **Использование Tailwind CSS**: Поскольку среда предпросмотра в iframe — это отдельный экземпляр Rsbuild, если ваши компоненты, отображаемые в предпросмотре, зависят от Tailwind CSS v4, настройте `@rsbuild/plugin-tailwindcss` в `iframeOptions.builderConfig.plugins` и внедрите вашу точку входа Tailwind CSS через `preEntry`.
...
Настраивает порт dev-сервера для предпросмотра в iframe. Если указанный порт уже занят, плагин автоматически попробует следующий порт, до 20 попыток.
...
Например, чтобы добавить поддержку Less или Sass в превью, установите и настройте соответствующий плагин Rsbuild:
...
- `@rsbuild/plugin-less` и `@rsbuild/plugin-sass` больше не встроены. Если вам нужна поддержка Less или Sass в превью, установите соответствующий плагин и настройте его через `iframeOptions.builderConfig`:
