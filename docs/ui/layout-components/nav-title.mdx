...
Этот компонент является частью панели навигации. `NavTitle` отображает логотип сайта и заголовок в левом верхнем углу панели навигации.

## Использование

Используйте его через [кастомную тему](/guide/basic/custom-theme). Компонент считывает конфигурацию из `rspress.config.ts` через [`useSite`](/ui/hooks/use-site):
