---
description: Включите обзорную страницу через секцию overview в метаданных, автоматически извлекая информацию из боковой панели для генерации содержимого обзора.
---

# Страница обзора

Обзорная страница отображает обзор всех статей в директории, автоматически извлекая информацию из боковой панели для создания сгруппированного контента.
...
Используйте [кастомную тему](/guide/basic/custom-theme) для изменения стилей компонента [OverviewGroup](/ui/layout-components/overview-group), продолжая использовать встроенную функциональность страницы обзора:
...
Используйте компонент [OverviewGroup](/ui/layout-components/overview-group) с `pageType: doc-wide` для настройки макета страницы и определения содержимого самостоятельно:
