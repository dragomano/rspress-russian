...
Когда существует корневой `_meta.json`, Rspress генерирует единую боковую панель (с ключом `'/'`) для всех страниц, независимо от того, какой элемент навигации активен. Корневой `_meta.json` служит точкой входа для всего дерева боковой панели, обычно ссылаясь на подкаталоги:
...
Чтобы удобнее редактировать файлы `_nav.json` и `_meta.json`, в Rspress предусмотрены два файла схем: `@rspress/core/meta-json-schema.json` и `@rspress/core/nav-json-schema.json`. Они обеспечивают подсказки типов в редакторе.
...
В директории guide можно настроить `_meta.json` следующим образом:
...
Для получения подробной информации об использовании обратитесь к [компоненту Tag](/ui/layout-components/tag).
