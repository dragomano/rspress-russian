...
- [ ] **markdown.mdxRs**: удалить опцию `markdown.mdxRs` (больше не поддерживается)
...
Следующие пакеты теперь встроены в `@rspress/core`. Если вы обновляетесь с версии 1.x, используйте таблицу изменений путей импортов выше, чтобы обновить пути импорта:
...
## [Важно] Функции, включенные по умолчанию

В V2 следующие функции включены по умолчанию:

| Функция                        | Описание                                                          |
| ------------------------------ | ----------------------------------------------------------------- |
| `markdown.link.checkDeadLinks` | Проверка мёртвых ссылок, исправление битых ссылок на основе логов |
| `search.codeBlocks`            | Результаты поиска включают блоки кода                             |
| `dev.lazyCompilation`          | Ленивая компиляция для более быстрого запуска в режиме разработки |
| `performance.buildCache`       | Постоянный кэш сборки для ускорения повторных сборок              |

:::tip
Если вы сталкиваетесь с ошибками мёртвых ссылок, лучше постарайтесь исправить битые ссылки, а не отключать функцию `markdown.link.checkDeadLinks`.
:::
...
## Удалена опция `markdown.mdxRs`

Опция `markdown.mdxRs` была удалена в V2. Rspress больше не использует MDX-парсер на основе Rust (`@rspress/mdx-rs`). Пожалуйста, удалите опцию `markdown.mdxRs` из вашей конфигурации.

Сохранение этой опции в конфигурации вызовет ошибки проверки типов TypeScript, но не повлияет на поведение во время выполнения.
...
export default defineConfig({
  markdown: {
    // опция mdxRs удалена, замена не требуется
    remarkPlugins: [plugin1, plugin2],
  },
});
```
...
`@rspress/plugin-preview` V2 больше не включает `@rsbuild/plugin-less` и `@rsbuild/plugin-sass` как встроенные зависимости. Если вам нужна поддержка Less или Sass в превью, установите соответствующий плагин и настройте его через `iframeOptions.builderConfig`:
...
Подробнее о миграции см. [руководство по миграции plugin-preview](/plugin/official-plugins/preview#миграция-с-v1).
