...
URL-адреса автоматически включают настроенные префиксы `siteOrigin`, [`base`](/api/config/config-basic#base), локали и версии. URL-адрес страницы в формате Markdown формируется на основе текущего маршрута. Без `siteOrigin` URL-адреса остаются путями с учётом `base`.
